<?php namespace System\Handlers;

use System\Databases\Database;
use System\Form\Data;
use System\Orders\Order;
use System\Reservations\Reservation;
//use System\Utils\Image;

/**
 * Class AddHandler
 * @package System\Handlers
 */
class ReservationEditHandler extends BaseHandler
{
    use ProductFillAndValidate;

    private Reservation $reservation;
    private $orders;

    public function initialize(): void
    {

        //Init the database
        $db = (new Database(DB_HOST, DB_USER, DB_PASS, DB_NAME))->getConnection();

        try {
            //Get the record from the db & execute POST logic

            $this->reservation = Reservation::getByCode($_GET['code'], $db);
            $reservation_id = $this->reservation->getId();

            $this->orders = Order::getByReservationId((int)$reservation_id,$db);

            $this->executePostHandler();

            //Database magic when no errors are found
            if (isset($this->formData) && empty($this->errors)) {
                //If image is not empty, process the new image file
//                if ($_FILES['image']['error'] != 4) {
//                    //Init image class
//                    $image = new Image();
//
//                    //Remove old image
//                    $image->delete($this->album->image);
//
//                    //Store new image & retrieve name for database saving (override current image name)
//                    $this->album->image = 'images/' . $image->save($_FILES['image']);
//                }

                //Save the record to the db
                if ($this->reservation->update($db)) {
                    $success = "Reservering bijgewerkt";
                } else {
                    //Probably should't be returned to a user..
                    $this->errors[] = "Database error info: " . $db->errorInfo();
                }
            }

            $pageTitle = 'Edit Reservering';
        } catch (\Exception $e) {
            //Probably should't be returned to a user..
            $this->errors[] = $e->getMessage();
            $pageTitle = 'Deze reservering bestaat niet!';
        }



        //Return formatted data
        $this->renderTemplate([
            'pageTitle' => $pageTitle,
            'reservation' => $this->reservation ?? false,
            'orders' => $this->orders ?? false,
            'success' => $success ?? false,
            'errors' => $this->errors
        ]);
    }
}